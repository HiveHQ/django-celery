...
class TaskStateManager(ExtendedManager):
    ...
    def delete_by_states(self, states, expires):
        info('delete_by_states: ' + str(states))
        info('expires: ' + str(expires))
        if expires is not None:
            info('expired: ' + str(self.expired(states, expires).query))
            return self.expired(states, expires).delete()
