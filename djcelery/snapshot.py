...
EXPIRE_PENDING = getattr(settings, 'CELERYCAM_EXPIRE_PENDING',
                         timedelta(days=5))
# Change frequency that expired tasks are deleted from the database
CLEANUP_FREQUENCY = getattr(settings, 'CELERYCAM_CLEANUP_FREQUENCY', 3600)
...
class Camera(Polaroid):
    ...
    def __init__(self, *args, **kwargs):
        kwargs['cleanup_freq'] = CLEANUP_FREQUENCY
        info('djcelery CLEANUP_FREQUENCY: ' + str(CLEANUP_FREQUENCY))
        super(Camera, self).__init__(*args, **kwargs)
        info('djcelery cleanup_freq: ' + str(self.cleanup_freq))
    ...
    def on_cleanup(self):
        info('Cleanup Called!')
        info('States: ' + str([x for state in self.expire_states.keys() for x in state]))
        return self.TaskState.objects.delete_by_states(states=[x for state in self.expire_states.keys() for x in state],
